...
function setTokensInCookie(res, token) {
    // put it in cookie
    res.cookie("learnbloomAccessCookie", token.accessToken, {
        maxAge: 1000 * 60 * 60 * 24 * 30, // 30 days
        httpOnly: true,
    });

    res.cookie("learnbloomRefreshCookie", token.refreshToken, {
        maxAge: 1000 * 60 * 60 * 24 * 30, // 30 days
        httpOnly: true,
    });
}
...
class AuthController {
    async refresh(req, res) {
        ...
        const { learnbloomRefreshCookie: refreshTokenFromCookie } = req.cookies;
    ...
    async logout(req, res) {
        const { learnbloomRefreshCookie } = req.cookies;
        await tokenService.removeToken(learnbloomRefreshCookie);

        res.clearCookie("learnbloomRefreshCookie");
        res.clearCookie("learnbloomAccessCookie");
